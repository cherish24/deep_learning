...
def train_rnn_cnn():
	...
	config = "./data"
	w2vmodel_file = os.path.join(config, "googleVector.bin")
	x_train, y_train, x_dev, y_dev, embedding_mat = data_help2.load_data2(w2vmodel_file, params['dev_size'])
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			train_batches = data_help2.batch_iter(list(zip(x_train, y_train)), params['batch_size'], params['num_epochs'])
			...
			for train_batch in train_batches:
				...
				if current_step % params['evaluate_every'] == 0:
					dev_batches = data_help2.batch_iter(list(zip(x_dev, y_dev)), params['batch_size'], 1)
			...
			# test_batches = data_help2.batch_iter(list(zip(x_test, y_test)), params['batch_size'], 1, shuffle=False)
